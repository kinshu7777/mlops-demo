...
def cmd_start(args):
    """
    The one-command version for first-time or new-copy setup: checks
    what's missing (data, a trained model, a drift check) and creates
    it automatically, then starts the API. This is the command to
    reach for whenever you've just unzipped or cloned a fresh copy of
    the project and don't want to remember the individual steps.
    """
    print(heading("=== Starting up ===\n"))

    if not os.path.exists(CONFIG["old_data_path"]):
        print(warn("No practice data found -> generating it..."))
        cmd_generate_data(args)
    else:
        print(ok("Practice data already exists."))

    model_files = glob.glob(os.path.join(CONFIG["models_dir_path"], "model_v*.joblib"))
    if not model_files:
        print(warn("No trained model found -> training one..."))
        cmd_train(args)
    else:
        print(ok(f"Found {len(model_files)} trained model version(s)."))

    if not os.path.exists(CONFIG["drift_report_path"]):
        print(warn("No drift check on record -> running one..."))
        cmd_check_drift(args)
    else:
        print(ok("Drift check already on record."))

    print(heading("\nEverything's ready. Starting the server...\n"))
    cmd_serve(args)
...
def build_parser():
    ...
    subparsers.add_parser("start", help="First-time setup + start the API, all in one command").set_defaults(func=cmd_start)
